...
// STRIKE SYSTEM
// ----------------------------
const STRIKES_FILE = "/app/data/strikes.json";
let strikes = {};
...
const BIRTHDAY_FILE = "/app/data/birthdays.json";
const SCHEDULE_FILE = "/app/data/schedules.json";
...
// ----------------------------
function buildLFGEmbed(member, game, platform, note, expiresAt, players) {
  const playerList = players.length > 0
    ? players.map(p => `• <@${p}>`).join("\n")
    : "*No one yet — be the first!*";
  return new EmbedBuilder()
    .setTitle(`🎮 Looking to Play — ${game}`)
    .setDescription(
      `**Host:** ${member}\n` +
      `**Platform:** ${platform}\n` +
      (note ? `**Notes:** ${note}\n` : "") +
...
async function setupLookingToPlayChannel() {
  ...
  if (botMsg) { console.log("ℹ️ Looking to play channel already exists"); return; }
  const embed = new EmbedBuilder()
    .setTitle("🎮 Looking to Play")
    .setDescription(
      "Want to find someone to game with? Post a looking-to-play request!\n\n" +
      "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n" +
      "**How it works:**\n\n" +
      "- Click **Looking to Play** below\n" +
      "- Fill in the game, platform, and any extra info\n" +
      "- Your post goes live and members can join in!\n" +
      "- Posts automatically expire after **24 hours**\n\n" +
      "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n" +
      "⚠️ **You can only have one active post at a time.**"
    )
    .setColor(0x57F287)
    .setFooter({ text: "DinoBot • Looking to Play" });
  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId("lfg_create").setLabel("Looking to Play").setEmoji("🎮").setStyle(ButtonStyle.Success)
  );
...
async function postBotInfoMessage(channel) {
  ...
  const embed2 = new EmbedBuilder()
    ...
    .setDescription(
      ...
      "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n" +
      "**🎮 How Looking to Play Works**\n\n" +
      "Head to `#🎮looking-to-play` and click **Looking to Play**:\n\n" +
      "- Fill in the game, platform, and any extra info\n" +
  ...
  try {
    ...
    if (customId === "raid_lift_lockdown") {
      ...
      await interaction.update({
        embeds: [EmbedBuilder.from(interaction.message.embeds[0]).setTitle("✅ Lockdown Lifted").setColor(0x57F287).setFooter({ text: `Lifted by ${interaction.user.tag} • DinoBot • Anti-Raid Protection` })],
        components: []
      });
      try {
        const modLogs = await client.channels.fetch(MOD_LOGS_CHANNEL_ID);
        await modLogs.send({
          embeds: [new EmbedBuilder()
            ...
            .setColor(0x57F287).setTimestamp().setFooter({ text: "DinoBot • Anti-Raid Protection" })]
        });
      } catch { /* ignore */ }
      return;
    }
    ...
    if (customId === "lfg_create") {
      ...
      if (activeLFG.has(user.id)) {
        await interaction.reply({ content: "❌ You already have an active looking-to-play post! It will expire automatically after 24 hours.", flags: 64 });
        return;
      }
      const modal = new ModalBuilder().setCustomId("lfg_modal").setTitle("🎮 Looking to Play");
      modal.addComponents(
        new ActionRowBuilder().addComponents(
          new TextInputBuilder().setCustomId("lfg_game").setLabel("What are you playing?").setStyle(TextInputStyle.Short).setPlaceholder("e.g. Minecraft, Phasmophobia, Warzone").setMaxLength(100).setRequired(true)
        ),
        new ActionRowBuilder().addComponents(
          new TextInputBuilder().setCustomId("lfg_platform").setLabel("Platform").setStyle(TextInputStyle.Short).setPlaceholder("e.g. PC, PS5, Xbox, Cross-platform").setMaxLength(50).setRequired(true)
        ),
        new ActionRowBuilder().addComponents(
          new TextInputBuilder().setCustomId("lfg_note").setLabel("Any extra info? (optional)").setStyle(TextInputStyle.Paragraph).setPlaceholder("e.g. Looking for 2-3 people, beginner friendly, rank, etc.").setMaxLength(300).setRequired(false)
        )
      );
      await interaction.showModal(modal);
      return;
    ...
    if (customId.startsWith("lfg_join_")) {
      ...
      const messageId = activeLFG.get(hostId);
      if (!messageId) {
        await interaction.reply({ content: "❌ This post has already expired.", flags: 64 });
        return;
      }
      try {
        const lfgChannel = await client.channels.fetch(LOOKING_TO_PLAY_CHANNEL_ID);
        const msg = await lfgChannel.messages.fetch(messageId);
        const embed = msg.embeds[0];
        let players = [];
        const matches = embed.description.match(/<@(\d+)>/g);
        if (matches) players = matches.map(m => m.replace(/<@|>/g, "")).filter(id => id !== hostId);
        if (players.includes(user.id)) {
          await interaction.reply({ content: "✅ You're already in!", flags: 64 });
          return;
        }
        if (user.id === hostId) {
          await interaction.reply({ content: "❌ You can't join your own post!", flags: 64 });
          return;
        }
        players.push(user.id);
        const hostMember = await guild.members.fetch(hostId).catch(() => null);
        const game = embed.title.replace("🎮 Looking to Play — ", "");
        const platformLine = embed.description.match(/\*\*Platform:\*\* (.+)/);
        const platform = platformLine ? platformLine[1] : "Unknown";
        const noteLine = embed.description.match(/\*\*Notes:\*\* (.+)/);
        const note = noteLine ? noteLine[1] : null;
        const expiresMatch = embed.description.match(/<t:(\d+):R>/);
        const expiresAt = expiresMatch ? parseInt(expiresMatch[1]) * 1000 : Date.now() + 24 * 60 * 60 * 1000;
        const newEmbed = buildLFGEmbed(hostMember, game, platform, note, expiresAt, players);
        const row = new ActionRowBuilder().addComponents(
          new ButtonBuilder().setCustomId(`lfg_join_${hostId}`).setLabel(`I'm In! 🎮 (${players.length})`).setStyle(ButtonStyle.Success)
        );
        await msg.edit({ embeds: [newEmbed], components: [row] });
        await interaction.reply({ content: `✅ You joined **${game}**! 🎮`, flags: 64 });
      } catch (err) {
        console.error("❌ Error joining LFG:", err);
        await interaction.reply({ content: "❌ Something went wrong.", flags: 64 });
      }
      return;
    }
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (customId === "lfg_modal") {
    ...
    const platform = interaction.fields.getTextInputValue("lfg_platform").trim();
    const note = interaction.fields.getTextInputValue("lfg_note")?.trim() || null;
    const expiresAt = Date.now() + 24 * 60 * 60 * 1000;
    try {
      const lfgChannel = await client.channels.fetch(LOOKING_TO_PLAY_CHANNEL_ID);
      const embed = buildLFGEmbed(member, game, platform, note, expiresAt, []);
      const row = new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId(`lfg_join_${user.id}`).setLabel("I'm In! 🎮 (0)").setStyle(ButtonStyle.Success)
      );
      const msg = await lfgChannel.send({ content: `🎮 ${member} is looking for people to play with!`, embeds: [embed], components: [row] });
      activeLFG.set(user.id, msg.id);
      await interaction.reply({ content: `✅ Your looking-to-play post is live! It expires in 24 hours. 🎮`, flags: 64 });
      setTimeout(async () => {
        try {
          activeLFG.delete(user.id);
          const expiredMsg = await lfgChannel.messages.fetch(msg.id).catch(() => null);
          if (!expiredMsg) return;
          const expiredEmbed = EmbedBuilder.from(expiredMsg.embeds[0])
            .setTitle(`❌ Expired — ${game}`)
            .setColor(0xED4245)
            .setFooter({ text: "DinoBot • Looking to Play • Expired" });
          await expiredMsg.edit({ embeds: [expiredEmbed], components: [] });
          console.log(`⏰ LFG post expired for ${user.tag}`);
        } catch (err) {
          console.error("❌ Error expiring LFG post:", err);
        }
      }, 24 * 60 * 60 * 1000);
      console.log(`✅ LFG post created by ${user.tag} — ${game} on ${platform}`);
    } catch (err) {
...
app.post("/twitch/eventsub", async (req, res) => {
  try {
    ...
    if (messageType === "notification" && req.body.subscription?.type === "stream.online") {
      ...
      if (!stream) { console.warn(`⚠️ Could not get stream info for ${broadcaster_user_name} after retries`); return res.status(200).send("Stream info unavailable"); }
